...
BuoyantObject::BuoyantObject(physics::LinkPtr _link)
{
  ...
  this->boundingBox = ignition::math::AxisAlignedBox(
      ignition::math::Vector3d(bBox.min.x, bBox.min.y, bBox.min.z),
      ignition::math::Vector3d(bBox.max.x, bBox.max.y, bBox.max.z));
#endif
...
{
  ignition::math::Vector3d size = this->boundingBox.Size();
  ...
  else
  {
    ...
    {
      this->waterLevelPlaneArea = size.X() * size.Y();
...
void BuoyantObject::SetBoundingBox(const ignition::math::AxisAlignedBox &_bBox)
{
  ...
  gzmsg << "New bounding box for " << this->link->GetName() << "::"
    << "Min: " << this->boundingBox.Min() << ", Max: " << this->boundingBox.Max() << std::endl;
}
