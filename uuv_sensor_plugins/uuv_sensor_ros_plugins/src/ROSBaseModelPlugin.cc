...
void ROSBaseModelPlugin::SendLocalNEDTransform()
{
  ...
  // Only publish if enough time has passed (e.g. 0.1 seconds)
  if ((current_time - last_publish_time).toSec() < 0.1)
    return;
    
  // Only publish if this is the primary publisher (based on namespace)
  if (this->robotNamespace.find("rexrov") == std::string::npos)
    return;
    
  geometry_msgs::TransformStamped msg;
  this->tfLocalNEDFrame.stamp_ = current_time;
